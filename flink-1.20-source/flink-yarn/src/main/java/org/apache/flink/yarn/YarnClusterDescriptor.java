...
public class YarnClusterDescriptor implements ClusterDescriptor<ApplicationId> {
    ...
    private ClusterClientProvider<ApplicationId> deployInternal(
            ...
            String yarnClusterEntrypoint, // AM启动类
            @Nullable JobGraph jobGraph,
            boolean detached)
            throws Exception {


        // Kerberos / Hadoop 安全校验（与 k8s 无关）：
        // UserGroupInformation 是 Hadoop 的登录用户抽象；若集群开了 Kerberos，
        // 则要求当前登录用户有有效的 Kerberos 凭据或 delegation token，否则直接拒绝提交。
        // 相关配置项都是 SecurityOptions.KERBEROS_*。
    ...
    /**
     * ★ 真正把 Flink 集群提交到 YARN 的地方。
     *
     * <p>调用方 {@code deployInternal()} 已经通过 {@code yarnClient.createApplication()} 拿到了
     * 一个空的 YARN Application；本方法负责把它的 AM 容器规格准备齐、提交出去、再轮询到集群就绪，
     * 最终返回 {@link ApplicationReport}。
     *
     * <p>全程按代码顺序可分 6 段：
     *
     * <ol>
     *   <li><b>初始化文件系统</b> —— Flink FS 抽象 + Hadoop FS + staging 目录
     *   <li><b>HA 与重试次数</b> —— zk namespace、{@code maxAppAttempts}
     *   <li><b>收集并上传文件</b> —— flink dist / 用户 jar / job.graph / flink-conf.yaml / 安全凭据，
     *       这一段占了方法大半篇幅，本质是在"填 local resource 清单"
     *   <li><b>拼 CLASSPATH 与环境变量</b> —— 决定 AM 容器里能看到什么
     *   <li><b>构造 AM 容器规格并提交</b> —— {@code setupApplicationMasterContainer()} + {@code submitApplication()}
     *   <li><b>轮询等待集群就绪</b> —— 每 250ms 查一次 ApplicationReport
     * </ol>
     *
     * <p>⚠️ <b>关键认知</b>：本方法<b>全程运行在客户端</b>。第 6 段那个 {@code while} 循环是
     * <b>客户端在等 AM 起来</b>，不是 AM 自己在跑。AM 容器里执行的是
     * {@code yarnClusterEntrypoint} 那个类（由第 5 段写进容器的启动命令）。
     *
     * @param configuration 已合并好的有效配置（flink-conf.yaml + CLI -D + 程序级配置）
     * @param yarnClusterEntrypoint AM 容器的启动类名，来自
     *     {@code getYarnSessionClusterEntrypoint()} / {@code getYarnJobClusterEntrypoint()} /
     *     {@code YarnApplicationClusterEntryPoint}
     * @param jobGraph per-job 模式下为作业图；session 模式传 null（集群起来后是空的）
     */
    // 启动ApplicationMaster
    private ApplicationReport startAppMaster(
            ...
            YarnClient yarnClient, // 客户端实例
            YarnClientApplication yarnApplication, // app对象
            ClusterSpecification clusterSpecification)
            throws Exception {
        ...
        // 序列化配置 传输到HDFS
        // 注意这里有两套 FS 抽象，别混淆：
        //   ① Flink 自己的 org.apache.flink.core.fs.FileSystem（下面这行初始化）
        //   ② Hadoop 的 org.apache.hadoop.fs.FileSystem（下一步 FileSystem.get(yarnConfiguration)）
        org.apache.flink.core.fs.FileSystem.initialize(
                configuration, PluginUtils.createPluginManagerFromRootFolder(configuration));

        final FileSystem fs = FileSystem.get(yarnConfiguration);

        // 如果 Hadoop FS 的 scheme 是 file://，说明集群里没配对分布式文件系统配置，
        // Flink 会把 jar/配置都传到本地磁盘——其他节点的容器根本读不到，必然失败。
        // 所以这里提前告警（GoogleHadoopFileSystem 没重写 getScheme()，故单独排除）。
        ...
        // 已在远端准备好的 lib / usrlib 目录：
        // 如果开了 yarn.provided-lib-dirs，就复用远端的依赖，跳过本地上传，
        // 这对"大量作业共用同一套依赖"的场景能省掉每次作业的重复上传。
        ...
        // staging 目录：本次提交在 HDFS 上的临时工作区，
        // 所有 jar / 配置文件 / job.graph 都先传到这里，再作为 local resource 分发给容器。
        // 部署失败时由 DeploymentFailureHook 负责删除。
        Path stagingDirPath = getStagingDir(fs);
        FileSystem stagingDirFs = stagingDirPath.getFileSystem(yarnConfiguration);

        // 上传器：统一管理"本地文件 → HDFS staging → 注册为 YARN local resource"这三步。
        // 后面所有 registerMultipleLocalResources / registerSingleLocalResource 都走它。
        ...
        // 给这次的 Application 生成 HA cluster id（即 zk 上的 namespace）。
        // 必须在 AM 启动前定下来，因为 AM 起来后要靠它去 zk 里选主；
        // 用 ApplicationId 做后缀，保证同一集群多次提交不会互相覆盖状态。
        // ------------------ Add Zookeeper namespace to local flinkConfiguration ------
        setHAClusterIdIfNotSet(configuration, appId);

        if (HighAvailabilityMode.isHighAvailabilityModeActivated(configuration)) {
            // 开了 HA 就允许 YARN 重试整个 Application（AM 挂了会在别的节点重起）。
            // 重试次数取自 yarn.application-attempts，默认用 YARN 的
            // DEFAULT_RM_AM_MAX_ATTEMPTS（通常是 2）。
            ...
            // 往 AM 的容器环境里塞 zk 相关的 HA 参数，AM 才知道去哪儿做选主。
            activateHighAvailabilitySupport(appContext);
        } else {
            // 没开 HA 就只跑一次：AM 挂了整个 Application 直接失败，
            // 因为没有 HA 存储来恢复作业状态，重试也没有意义。
        ...
        // ==================== 第 3 段：收集并上传文件 ====================
        // 这一段的本质是"填 local resource 清单"：凡是 AM / TM 容器里要用到的东西，
        // 都得先上传到 HDFS staging 目录，再注册成 YARN local resource。

        // 用户 jar 有两个来源，取决于部署模式：
        //   per-job / session：jar 信息已经在 JobGraph 里（jobGraph.getUserJars()）
        //   application     ：客户端还没解析出 JobGraph（main 在 AM 里才跑），
        //                     所以只能从配置项 pipeline.jars 拿（见下面 jarUrls 那段）
        ...
        // 上传flink 相关jar包到hdfs
        // 仅 application 模式会走这里：此时 jar 由 CLI 以管道配置命令传递，
        // 需要额外读 pipeline.jars 才能拿到用户 jar 列表。
            ...
            for (Map.Entry<String, DistributedCache.DistributedCacheEntry> entry :
                ...
                if (!Utils.isRemotePath(entry.getValue().filePath)) {
                    ...
                    Tuple2<Path, Long> remoteFileInfo =
                            fileUploader.uploadLocalFileToRemote(localPath, entry.getKey()); // 上传
        ...
        // ★ 这里体现了 local resource 的两种可见性（YARN 概念）：
        //     PUBLIC      —— 集群内所有应用都能复用，走 provided lib 目录（预置在 HDFS 上的依赖）
        //     APPLICATION —— 只有本次 Application 可见，是临时上传的那批
        // 用 provided lib 能省掉每个作业重复上传整套依赖的开销。
        ...
        // ★ 又一类文件："只 ship、不进 classpath"。
        //    插件（plugins 目录）就是典型：它由 Flink 自己的插件加载机制从 plugins/ 目录读取，
        //    不能塞进 classpath，否则会和 Flink 自带依赖冲突。
        //    这解释了为什么它单独调一次 registerMultipleLocalResources 却不把结果加进 systemClassPaths。
        ...
        // archive 类型的资源会被 YARN 自动解压到容器工作目录
        ...
        if (YarnApplicationClusterEntryPoint.class.getName().equals(yarnClusterEntrypoint) //判断类型是否为YarnApplicationClusterEntryPoint
        ...
        // Upload and register user jars 上传用户jar包
        // 落到哪个目录由 yarn.user-jar-inclusion 决定：
        //   DISABLED → 放进 usrlib/（不进 classpath，由 Flink 的 usrlib 机制统一加载）
        //   其它值   → 放在容器工作目录（会进 classpath）
        ...
        // ★ yarn.user-jar-inclusion 的取值决定用户 jar 在 CLASSPATH 里的位置，
        //   本质是解决"用户 jar 与 Flink 自带 jar 冲突时谁优先"：
        //     FIRST    → 用户 jar 放最前（可覆盖 Flink 自带的同名类）
        //     LAST     → 用户 jar 放最后（Flink 自带优先，用户依赖只补缺）
        //     ORDER    → 保持上传顺序（下一行直接并进 systemClassPaths）
        //     DISABLED → 完全不进 classpath（走 usrlib）
        if (userJarInclusion == YarnConfigOptions.UserJarInclusion.ORDER) {
            systemClassPaths.addAll(userClassPaths);
        }

        // 排序让 classpath 内容稳定可复现——否则同一份作业两次提交可能生成不同的容器规格，
        // 影响问题定位与对比。
        ...
        // 从配置算出 JM 进程的内存规格（总进程内存 → 堆/堆外/元空间/JVM overhead 的拆分）。
        // 这就是 jobmanager.memory.process.size 在客户端被消费的地方。
        final JobManagerProcessSpec processSpec =
                JobManagerProcessUtils.processSpecFromConfigWithNewOptionToInterpretLegacyHeap(
                        flinkConfiguration, JobManagerOptions.TOTAL_PROCESS_MEMORY);

        // ★ setupApplicationMasterContainer 内部把 yarnClusterEntrypoint 这个类名
        //   写进 startCommandValues.put("class", ...)，最终成为容器的启动命令：
        //       java ... <YarnJobClusterEntrypoint>
        //   所以「AM 容器里跑的是 entrypoint」这件事，就是在这里确定的。
        //   此时只是"拼命令字符串"，还没有任何容器被创建。
        final ContainerLaunchContext amContainer =
                setupApplicationMasterContainer(yarnClusterEntrypoint, hasKrb5, processSpec);

        // 把 Delegation Token 塞进容器环境：容器里的进程用它在 Kerberos 集群上访问 HDFS/YARN，
        // 避免每个容器各自去 kinit。
        ...
        // 把前面注册过的所有 local resource 一次性绑定到容器上。
        // 到这一行，fileUploader 收集的清单就变成了容器规格的一部分；随后即可关闭上传器。
        ...
        // ★ AM 容器自身的资源规格（注意：这是 AM 的资源，不是 TaskManager 的）。
        //   内存来自 clusterSpecification.getMasterMemoryMB()，它由
        //   YarnClusterClientFactory.getClusterSpecification() 从 jobmanager.memory.process.size 算出；
        //   核数来自 yarn.appmaster.vcores。
        //   这正是"per-job/application 模式下 -Djobmanager.memory.process.size 能生效"的原因。
        ...
        // 把前面所有准备工作挂到 ApplicationSubmissionContext 上。
        // 走到这里，一个完整的"应用提交描述"才真正成型。
        appContext.setApplicationName(customApplicationName);
        appContext.setApplicationType(applicationType != null ? applicationType : "Apache Flink");
        appContext.setAMContainerSpec(amContainer);
        appContext.setResource(capability);

        // 以下都是 YARN 侧的调度属性：优先级、队列、node label、tags
        ...
        // ★ 注册一个 JVM shutdown hook 兜底：万一客户端在提交过程中挂掉/被 Ctrl+C，
        //   由它负责 kill 掉半成品 Application 并删除 HDFS 上的 staging 目录。
        //   注意它内部会**另建一个 YarnClient**（因为描述符持有的那个可能已经被 close() 掉了）。
        ...
        // ★★ 真正的"提交"就这一行：把 ApplicationSubmissionContext 交给 YARN ResourceManager。
        //    返回时只是"提交被受理"，AM 容器还没起来，更别说作业开始跑了。
        yarnClient.submitApplication(appContext);

        // ==================== 第 6 段：轮询等待集群就绪 ====================
        // ⚠️ 这个循环运行在**客户端**。它在等 AM 容器被 YARN 调度起来并进入 RUNNING。
        //    AM 容器里执行的是 yarnClusterEntrypoint，与本循环无关。
        ...
        while (true) {
            ...
            switch (appState) {
                case FAILED:
                case KILLED:
                    // 失败时把 YARN 给的 diagnostics 一并抛出，并提示用 yarn logs 深入排查
                    throw new YarnDeploymentException(
                    ...
                    // break ..
                case RUNNING:
                    // 集群起来了。注意此时作业可能还没跑完（per-job 的作业在 AM 内部提交）。
                    LOG.info("YARN application has been deployed successfully.");
                    break loop;
                case FINISHED:
                    // 应用直接结束了：通常是作业跑得极快，或 AM 启动即失败但状态已收敛。
                    LOG.info("YARN application has been finished successfully.");
                    break loop;
                default:
                    // NEW / NEW_SAVING / SUBMITTED / ACCEPTED 等中间态：继续轮询
                    if (appState != lastAppState) {
                        LOG.info("Deploying cluster, current state " + appState);
                    }
                    // 超过 60s 还没就绪就打一条提示——通常意味着 YARN 队列资源不足
        ...
        // 部署成功，撤掉兜底 hook，避免客户端正常退出时误杀正在运行的集群
