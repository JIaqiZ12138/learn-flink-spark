...
public class ActiveResourceManager<WorkerType extends ResourceIDRetrievable>
    ...
    /**
     * ★★ "主动型" ResourceManager 的初始化 —— YARN / K8s 走这条。
     *
     * <p>与 {@code StandaloneResourceManager.initialize()}（只开个启动期）不同，
     * 这里要把具体的资源提供者驱动初始化起来：
     * {@code YarnResourceManagerDriver.initializeInternal()} 会创建
     * AMRMClientAsync / NMClientAsync，并调用 {@code registerApplicationMaster()}
     * 向 YARN ResourceManager 注册 —— <b>这一步完成，本进程在 YARN 眼里才真正成为 AM。</b>
     */
    @Override
