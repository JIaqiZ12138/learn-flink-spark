...
public abstract class ResourceManager<WorkerType extends ResourceIDRetrievable>
    ...
    /**
     * ★ RM 的实际启动动作（由 {@code ResourceManager.onStart()} 调用）。
     *
     * <p>注意最后那个 {@code initialize()} 是<b>抽象方法</b>，是实现类的多态点：
     * {@code StandaloneResourceManager} 只开一个启动期；
     * {@code ActiveResourceManager} 则会去初始化 {@code ResourceManagerDriver}——
     * 对 YARN 来说就是 {@code YarnResourceManagerDriver.initializeInternal()}，
     * 那里会创建 AMRMClientAsync/NMClientAsync 并调用
     * {@code registerApplicationMaster()} 向 YARN 注册，<b>AM 到此正式上线</b>。
     */
